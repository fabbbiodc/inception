...
if [ -d "$DATADIR/mysql" ]; then
	...
	mariadb -u root <<-EOF
		...
		CREATE USER IF NOT EXISTS '$DB_USER'@'wordpress.inception_inception-net' IDENTIFIED BY '$DB_PASSWORD';
		CREATE USER IF NOT EXISTS '$DB_USER'@'wordpress' IDENTIFIED BY '$DB_PASSWORD';
		GRANT ALL PRIVILEGES ON \`$DB_NAME\`.* TO '$DB_USER'@'%';
		GRANT ALL PRIVILEGES ON \`$DB_NAME\`.* TO '$DB_USER'@'wordpress.inception_inception-net';
